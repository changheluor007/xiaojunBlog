...
def cache_decorator(expiration=3*60):
    ...
    def wrapper(func):
        ...
        def news(*args, **kwargs):
            ...
            print('-------------')
            print("unuque_str:%s"%(unique_str))
            m = md5.new(unique_str)
            key = m.hexdigest()
            value = cache.get(key)
            print("value:%s...key:%s"%(value,key))
            if value:
                return value
            else:
...
#from https://github.com/the5fire/Python-LRU-cache
class LRUCacheDict(object):
    def __init__(self, max_size=1024, expiration=15*60):
        ...
        print('进缓存了.................%s,%s,%s,%s,%s'%(self.max_size,self.expiration,self.__values,self.__expire_times,self.__access_times))
