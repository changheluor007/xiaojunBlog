...
from xiaojunBlog import settings
...
class Category(models.Model):
    ...
    desc = models.CharField(max_length= 100, blank= True, verbose_name=u"点击分类后是否显示")

    rank = models.IntegerField(default= 0, verbose_name=u"展示排序")
    status = models.IntegerField(default=0, choices=STATUS.items(),verbose_name=u"展示状态")
...
class Post(models.Model):
    ...
    alias = models.CharField(max_length= 100, db_index=True ,blank= True,null=True,verbose_name=u'英文标题',help_text=u'做伪静态url')
    ...
    tags = models.CharField(max_length=100 ,null= True , blank= True ,verbose_name=u'标签', help_text=u'用,进行分割')
    status = models.IntegerField(default=0,choices=STATUS.items(),verbose_name=u'状态')

    is_old= models.BooleanField(default=False ,verbose_name=u'是否是旧数据')
    ...
    def get_recently_posts(cls,num):
        return cls.objects.values('title','alias').filter(status=0).order_by('-create_time')[:num]

    @classmethod
    @cache_decorator(1*60)
    def get_hots_posts(cls,num):
        return cls.objects.values('title','alias').filter(status=0).order_by('-view_times')[:num]
    ...
    def related_posts(self):
        ...
        if not related_posts:
            related_posts = Post.objects.values('title','alias').\
                filter(category=self.category).exclude(id=self.id)[:10]
...
class Page(models.Model):
    ...
    author = models.ForeignKey(User,verbose_name=u'作者')
    title = models.CharField(max_length= 100,verbose_name=u'标题')
    alias = models.CharField(max_length=100,blank=True,null=True,verbose_name=u'英文标题', help_text=u'做伪静态')
    ...
    link = models.CharField(max_length=200,blank=True,null=True,verbose_name=u'链接',help_text=u'该链接存在时其他内容无效')
    rank = models.IntegerField(default=1, verbose_name=u'排序',help_text=u'从右开始')
    status = models.IntegerField(default=0,choices=STATUS.items(),verbose_name=u'状态')
    ...
    class Meta:
        ordering = ['-rank','-create_time']
        verbose_name_plural=verbose_name=u'界面'


  #右侧的插件
class Widget(models.Model):
    title = models.CharField(max_length=100,verbose_name='标题')
    content =  models.TextField(verbose_name=u'widget内容',help_text=u'html 不被转义')
